...
module.exports = class SubscriptionValidation {
    
    async createOne(req, res, next) {
        console.log('SubscriptionValidation@createOne');
    ...
    async updateOne(req, res, next) {
        console.log('SubscriptionValidation@updateOne');
        if (!req.params.id || req.params.id === '') {
            return response.badRequest('Id required.', res, false);
        }
        // make sure the user exists
        let user = await _Subscription.getOne(req.params.id);
