...
module.exports = class GoogleServices {
    async getAddress(address = null, city = null, country = null, businessName = null) {
        ...
        let searchAddress = encodeURIComponent(`${address}+${city}+${country}`);

        let googleAddress;

        try {
            console.log("Google Address :", searchAddress);
            googleAddress = await axios.get(`${process.env.GOOGLE_GEOCODE_URL}/json?address=${searchAddress}&key=${process.env.GOOGLE_MAPS_KEY}`, {});

        } catch (err) {
            console.log('ERROR GETTING GOOGLE ADDRESS: ', err);
        ...
        let fullAddress = {
            ...
            geometry: { type: 'Point', coordinates: [googleData.results[0].geometry.location.lng, googleData.results[0].geometry.location.lat]}
        };
    ...
    async getPlaceDetails(placeId = null) {
        ...
        let placeDetails;
        try {
            placeDetails = await axios.get(`${process.env.GOOGLE_PLACE_URL}/details/json?key=${process.env.GOOGLE_MAPS_KEY}&place_id=${placeId}`, {});
        ...
        console.log("Google Data :", googleData.result.address_components)
        ...
        let fullAddress = {
            ...
            geometry: { type: 'Point', coordinates: [googleData.result.geometry.location.lng,googleData.result.geometry.location.lat]}
        };
    ...
    async getAutoCompleteAddress(place){
        console.log("GoogleServies@getAutoCompleteAddress", process.env.GOOGLE_MAPS_KEY);
        if (!place ) {
            throw new Error('place is required for search');
        }

        let googleAddresses;

        try {
            googleAddresses = await axios.get(`${process.env.GOOGLE_PLACE_URL}/autocomplete/json?input=${place}&key=${process.env.GOOGLE_MAPS_KEY}`, {});

        } catch (err) {
            console.log('ERROR GETTING getAutoCompleteAddress: ', err.response);
            throw new Error('error: ', err.response)
        }

        console.log("Google Address :", googleAddresses)

        return  googleAddresses.data.predictions;
    }
}